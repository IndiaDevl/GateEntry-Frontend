...
//   fetchGateEntriesByPO // Add this import
// } from "../../api";
// import { useLocation } from "react-router-dom";
// import "./CreateHeader.css";
...
//       initialState[`VendorInvoiceWeight${suffix}`] = "";
//       initialState[`BalanceQty${suffix}`] = "";
//       initialState[`PurchaseOrderItem${suffix}`] = ""; // Add item number field
//       initialState[`OrderQuantity${suffix}`] = ""; // Add original order quantity
//     }
...
//   // Function to fetch and show PO dropdown
//   const fetchAndShowPODropdown = async (query, suffix) => {
//     setPoDropdown(prev => ({ ...prev, loading: true, show: true, suffix, searchQuery: query }));
//     try {
//       const response = await fetchPurchaseOrderSuggestions(query);
...
//   // Function to fetch and show PO line items with remaining calculation
//   const fetchAndShowPOItems = async (poNumber, suffix, shouldUpdateField = false) => {
//     try {
//       if (shouldUpdateField) {
//         setHeader(prev => ({ 
...
//       // Calculate remaining for each line item
//       const itemsWithRemaining = await Promise.all(items.map(async (item) => {
//         const material = item.Material || item.material || '';
//         const itemNo = item.PurchaseOrderItem || item.purchaseOrderItem || '';
//         const orderQty = toNum(item.OrderQuantity || item.orderQuantity || 0);
        
//         // Get initial remaining from localStorage
//         let initialRemaining = getRemainingFor(poNumber, material, itemNo);
        
//         // If not in localStorage, fetch all gate entries for this PO and calculate used quantity
//         if (!Number.isFinite(initialRemaining)) {
//           try {
//             // Fetch gate entries for this PO
//             const gateEntriesResponse = await fetchGateEntriesByPO(poNumber);
//             const gateEntries = gateEntriesResponse?.data?.d?.results || 
//                                 gateEntriesResponse?.data?.value || 
//                                 [];
            
//             // Calculate total used quantity for this specific line item
//             let usedQty = 0;
//             gateEntries.forEach(entry => {
//               // Check all 5 PO positions
//               for (let i = 1; i <= 5; i++) {
//                 const s = i === 1 ? '' : String(i);
//                 const entryPo = entry[`PurchaseOrderNumber${s}`];
//                 const entryMaterial = entry[`Material${s}`];
//                 const entryItemNo = entry[`PurchaseOrderItem${s}`] || '';
//                 const entryWeight = toNum(entry[`VendorInvoiceWeight${s}`] || entry[`BalanceQty${s}`] || 0);
                
//                 if (entryPo === poNumber && entryMaterial === material && 
//                     (!itemNo || entryItemNo === itemNo || entryItemNo === '')) {
//                   usedQty += entryWeight;
//                 }
//               }
//             });
            
//             initialRemaining = Math.max(0, orderQty - usedQty);
            
//             // Save to localStorage for future use
//             setRemainingFor(poNumber, material, itemNo, initialRemaining);
            
//           } catch (err) {
//             console.error('Error calculating used quantity:', err);
//             initialRemaining = orderQty; // Default to full quantity if can't calculate
//           }
//         }
...
//           RemainingQty: initialRemaining,
//           IsAvailable: initialRemaining > 0
//         };
//       }));

//       // Filter items with available quantity
//       const availableItems = itemsWithRemaining.filter(item => item.IsAvailable);
...
//     const itemNo = item.PurchaseOrderItem || item.purchaseOrderItem || '';
//     const orderQtyNum = toNum(item.OrderQuantity || item.orderQuantity);
//     const remainingQty = item.RemainingQty || orderQtyNum;
...
//         [`Material${suffix}`]: material,
//         [`MaterialDescription${suffix}`]: materialDesc,
//         [`PurchaseOrderItem${suffix}`]: itemNo, // Store item number
//         [`Vendor${suffix}`]: supplierCode,
//         [`VendorName${suffix}`]: supplierName,
//         [`OrderQuantity${suffix}`]: orderQtyNum, // Store original order quantity
//         [`BalanceQty${suffix}`]: remainingQty === 0 ? '' : String(remainingQty || '')
//       };
//     });
//   };
...
//       const remaining = getRemainingFor(poNo, material, itemNo);
//       const weight = toNum(header[`VendorInvoiceWeight${s}`]);

//       if (Number.isFinite(remaining) && remaining <= 0 && Number.isFinite(weight) && weight > 0) {
//         validationErrors.push(`PO ${poNo} / Item ${itemNo || material} is fully used (remaining 0). Remove weight or choose another line.`);
//       }
//       if (Number.isFinite(remaining) && Number.isFinite(weight) && weight > remaining) {
//         validationErrors.push(`PO ${poNo} / Item ${itemNo || material}: entered weight ${weight} exceeds remaining ${remaining}.`);
//       }
//     }
//     if (validationErrors.length) {
//       setError(validationErrors.join(' | '));
...
//       // Update remaining quantities - LINE ITEM WISE
//       for (let i = 1; i <= 5; i++) {
//         const s = i === 1 ? '' : String(i);
//         const poNo = header[`PurchaseOrderNumber${s}`];
//         const material = header[`Material${s}`];
...
//         const prevRemaining = getRemainingFor(poNo, material, itemNo);
//         const formBalance = toNum(header[`BalanceQty${s}`]);
//         const received = toNum(header[`VendorInvoiceWeight${s}`]);
...
//         // Also update at material level for backward compatibility
//         setRemainingFor(poNo, material, '', newRemaining);
//       }
...
//           {Array.from({ length: 5 }).map((_, idx) => {
//             const suffix = idx === 0 ? "" : String(idx + 1);
//             const poNo = header[`PurchaseOrderNumber${suffix}`];
//             const material = header[`Material${suffix}`];
//             const itemNo = header[`PurchaseOrderItem${suffix}`];
//             const remainingQty = getRemainingFor(poNo, material, itemNo);
...
//                       readOnly
//                       style={{ backgroundColor: '#f3f4f6', fontSize: '0.9em' }}
...
//                           fontSize: '0.8em',
//                           fontWeight: 'normal',
//                           color: remainingQty > 0 ? '#059669' : '#dc2626'
//                         }}>
...
//                       const usedQty = orderQty - remaining;
//                       const percentAvailable = orderQty > 0 ? Math.round((remaining / orderQty) * 100) : 0;
...
//                             {item.PurchaseOrderItemText || item.MaterialDescription || item.materialDescription || '-'}
//                           </td>
//                           <td className="text-right">{orderQty.toLocaleString()}</td>
//                           <td className="text-right" style={{ color: '#999' }}>
...
//                                   backgroundColor: percentAvailable > 50 ? '#10b981' : 
//                                                  percentAvailable > 20 ? '#f59e0b' : '#ef4444'
//                                 }}
//                               />
//                               <span className="progress-text">{percentAvailable}%</span>
//                             </div>
//                           </td>
//                           <td>
//                             <button
...
// // Utility functions for line-item-wise tracking
// const poItemKey = (poNo, material, itemNo) => 
//   `${String(poNo || '').trim()}__${String(material || '').trim()}__${String(itemNo || '').trim()}`;
...
//   // First try to get by item number (if available)
//   if (itemNo) {
//     const itemKey = poItemKey(poNo, material, itemNo);
//     const itemValue = map[itemKey];
//     if (typeof itemValue === 'number') return itemValue;
//   }
  
//   // Fallback to material-level (backward compatibility)
//   const materialKey = `${String(poNo || '').trim()}__${String(material || '').trim()}`;
//   const materialValue = map[materialKey];
//   return typeof materialValue === 'number' ? materialValue : (materialValue != null ? Number(materialValue) : null);
// };

// const setRemainingFor = (poNo, material, itemNo = '', qty) => {
//   try {
//     const map = getRemainingMap();
//     const key = itemNo ? poItemKey(poNo, material, itemNo) : `${String(poNo || '').trim()}__${String(material || '').trim()}`;
...
//     localStorage.setItem('poRemainingMap', JSON.stringify(map));
//   } catch (err) {
//     console.error('Error saving remaining qty:', err);
//   }
// };
